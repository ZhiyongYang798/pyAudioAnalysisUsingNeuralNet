...
def one_hot_encode(labels, featuresNew):
    n_unique_labels = len(labels)
    
    n_up = len(featuresNew[0])
   
    n_down = len(featuresNew[1])
    
    one_hot_encode_up = numpy.zeros((n_up,n_unique_labels))
    one_hot_encode_down = numpy.zeros((n_down,n_unique_labels))
    one_up = numpy.zeros((n_up,), dtype=numpy.int)
    one_down = numpy.ones((n_down,), dtype=numpy.int)
    one_hot_encode_up[numpy.arange(n_up), one_up] = 1
    one_hot_encode_down[numpy.arange(n_down), one_down] = 1
    return numpy.concatenate((one_hot_encode_up, one_hot_encode_down), axis=0)

def trainNeuralNet(features_NN, classNames_NN, modelName): #Working on

    training_epochs = 5000
    n_dim = features_NN.shape[1]
    n_classes = 2
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(training_epochs): 
            ...
            _,cost = sess.run([optimizer,cost_function],feed_dict={X:features_NN,Y:classNames_NN})
            cost_history = numpy.append(cost_history,cost)
...
def featureAndTrain(listOfDirs, mtWin, mtStep, stWin, stStep, classifierType, modelName, computeBEAT=False, perTrain=0.90):
    ...
    features_NN = numpy.concatenate((featuresNew[0], featuresNew[1]), axis=0)
    classNames_NN = one_hot_encode(classNames, featuresNew)
    ...
    if classifierType == "svm":
        ...
        trainNeuralNet(features_NN, classNames_NN, modelName)
